...
def list_installed(skip: set[str] | None = None) -> list[InstalledPackage]:
    """列出当前环境中已安装的包（按名称排序，同一归一化名只保留一个）。

    editable 安装（pip install -e .）时，importlib.metadata 可能同时枚举到
    site-packages 下的 dist-info 与源码目录中的 egg-info（二者指向同一项目），
    同一包会出现两次；这里按归一化名去重，优先保留 site-packages 中的条目。
    """
    skip = skip or SKIP_NAMES
    by_name: dict[str, InstalledPackage] = {}
    for dist in sorted(
        metadata.distributions(), key=lambda d: str(getattr(d, "_path", "") or "")
    ):
        name = _normalize_name(dist.metadata.get("Name") or "")
        if not name or name in skip:
            continue
        if name not in by_name:
            by_name[name] = InstalledPackage(
                name=name,
...
packages = []
seen = set()
for dist in sorted(metadata.distributions(), key=lambda d: str(getattr(d, "_path", "") or "")):
